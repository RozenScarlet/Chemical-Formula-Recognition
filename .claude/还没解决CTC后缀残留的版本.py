...
def train_epoch(model, train_loader, criterion, optimizer, scaler, device, label_converter, epoch, semantic_weight=0.05):
    """训练一个epoch"""
    model.train()
    ...
    with torch.no_grad():
        for batch_idx, (images, targets, target_lengths, raw_texts) in enumerate(tqdm(val_loader, desc='保存测试预测')):
            ...
            for i in range(batch_size):
                ...
                filtered_pred = []
                prev_char = -1
                for char in pred_seq:
                    if char != prev_char and char != 0:  # 0是CTC的blank字符
                        filtered_pred.append(char)
                    prev_char = char
                
                # 转换为文本
                pred_text = ''.join([label_converter.character[idx] for idx in filtered_pred 
                                   if idx < len(label_converter.character)])
                
                # 解码真实标签
                true_seq = targets[i].cpu().numpy()
                true_seq = [idx for idx in true_seq if idx != 1]  # 1是SPACE字符，用于填充
                true_text = ''.join([label_converter.character[idx] for idx in true_seq 
                                   if idx < len(label_converter.character)])
...
def main():
    ...
    parser.add_argument('--lr', type=float, default=5e-5/clear, help='学习率')
    ...
    parser.add_argument('--semantic-weight', type=float, default=0.0001, help='语义指导损失权重')
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_loss, train_char_acc, train_seq_acc = train_epoch(
            model, train_loader, criterion, optimizer, scaler, device, label_converter, epoch, semantic_weight=current_semantic_weight
        )
